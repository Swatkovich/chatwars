...
resources['Swatkovich HUY'] = path.join(__dirname, './data/result0.json');
//Samael HUY 29.08
resources['Samael HUY'] = path.join(__dirname, './data/result1.json');
//George`s HUY 29.08
resources['George228,George,George HUY,George`s HUY'] = path.join(
  __dirname,
  './data/result2.json'
);
//Gordo HUY 29.08 29.08
resources['Gordo HUY,Povar51345e,povar'] = path.join(
  __dirname,
  './data/result3.json'
);
//Evendream HUY 29.08
resources['Evendream HUY,Evendream'] = path.join(
  __dirname,
  './data/result4.json'
);
//dextrose 29.08
resources['dextrose'] = path.join(__dirname, './data/result5.json');
//VAGODROCHER HUY 29.08
resources['VAGODROCHER HUY'] = path.join(__dirname, './data/result6.json');
//BOLOTOSOSAT 29.08
resources['BOLOTOSOSAT'] = path.join(__dirname, './data/result7.json');
...
async function saveExploration(explorationData) {
  ...
  if (dataBaseRegion) {
    ...
    updatedDataBaseRegion.successQuestsNumber +=
      explorationData.successQuestsNumber;
    ...
    if (explorationData.resources.size > 0) {
      ...
      updatedDataBaseRegion.resources = sortResources(
        updatedDataBaseRegion.resources
      );
    }
...
function findExplorationCompleteMessages(messagesText) {
  ...
  for (let i = 0; i < messagesText.length; i++) {
    ...
    if (
      ...
      (message[0].includes('Exploration Complete!') ||
        message[0].includes('Исследование завершено!'))
    ) {
      while (!regionName) {
        ...
        if (
          ...
          (previousMessage[0].includes('You have reached your destination') ||
            previousMessage[0].includes('Ты прибыл в пункт назначения.'))
        ) {
...
function parseExplorations(explorationsComplete) {
  ...
  for (let exploration of explorationsComplete) {
    ...
    if (
      ...
      (exploration[2].includes('Nothing interesting happened') ||
        exploration[2].includes('Ничего интересного не произошло'))
    ) {
      parsedExplorations.push(parsedExploration);
      continue;
    }
    if (exploration[0].includes('✔️')) {
      parsedExploration.successQuestsNumber =
        exploration[0].split('✔️').length - 1;
      parsedExploration.questsNumber = parsedExploration.successQuestsNumber;
    }
    if (exploration[0].includes('▫️')) {
      let failQuestsNumber = exploration[0].split('▫️').length - 1;
      parsedExploration.questsNumber =
        parsedExploration.questsNumber + failQuestsNumber;
    }
    if (exploration[3]) {
    ...
    if (
      exploration[6].includes('Earned') ||
      exploration[6].includes('Ты получил')
    ) {
      let j = 7;
